...
public class View_user_doubleparking extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.view_double_user);

        Button loginButton = findViewById(R.id.button22);
        ...
        if (aptId != -1) {
            ...
            Cursor cursor = db.rawQuery("SELECT * FROM DoubleParkingUserInfo WHERE apt_id = ?", new String[]{String.valueOf(aptId)});
            ...
            if (cursor.moveToFirst()) {
                ...
                do {
                    ...
                    // 텍스트뷰에 정보 추가
                    userInfo.append("이중 주차 구역: ").append(parkingArea)
                ...
                userInfoTextView.setText("아파트에 이중 주차 중인 주민이 없습니다.");
            }
            // 커서와 데이터베이스 닫기
            cursor.close();
            db.close();
